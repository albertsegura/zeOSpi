...
seos.bin: system build user
	$(OBJCOPY) system system.out
	$(OBJCOPY) user user.out
		./build system.out user.out > seos.bin
...
system: system.o system.lds $(SYSOBJ)
	$(LD) $(LINKFLAGS) $(SYSLDFLAGS) -o $@ $< $(SYSOBJ) $(LIBSEOS) 
...
clean:
	rm -f *.o *.s bochsout.txt parport.out system.out system seos.bin user user.out *~ build 

debug: seos.bin
	qemu-system-arm -s -S -kernel seos.bin -cpu arm1176 -m 256 -M versatilepb -no-reboot &
